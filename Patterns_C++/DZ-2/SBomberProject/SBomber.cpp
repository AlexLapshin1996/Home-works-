...
void SBomber::CheckBombsAndGround() 
{
    ...
    for (size_t i = 0; i < vecBombs.size(); i++)
    {
        if (vecBombs[i]->GetY() >= y) // Пересечение бомбы с землей
        {
            ...
            DeleteDynamObjCommand* cmnd = new DeleteDynamObjCommand(vecDynamicObj, vecBombs[i]);
            cmnd->Execute();
            delete cmnd;
        }
    }
...
void SBomber::CheckDestoyableObjects(Bomb * pBomb)
{
    ...
    for (size_t i = 0; i < vecDestoyableObjects.size(); i++)
    {
        ...
        if (vecDestoyableObjects[i]->isInside(x1, x2))
        {
            ...
            DeleteStaticObjCommand* cmnd = new DeleteStaticObjCommand(vecStaticObj, vecDestoyableObjects[i]);
            cmnd->Execute();
            delete cmnd;
        }
    }
...
void SBomber::ProcessKBHit()
{
    ...
    case 'b':
    {
        DropBombCommand* command = new DropBombCommand(FindPlane(), vecDynamicObj, bombsNumber);
        command->Execute();
        delete command;
        break;
    }
